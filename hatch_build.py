...
- OPENMC_PYPI_PLAT:  wheel プラットフォームタグ (win_amd64 / linux_x86_64 /
  macosx_11_0_arm64 など。タグの正当性はビルド手順側の責任)。python パッケージは
  pure Python (ext_modules 無し) なので CPython バージョンに依存しない py3-none-<plat>。
- OPENMC_PYPI_OUT:   per-target 出力ディレクトリ (out/<triple>)。libopenmc と実行ファイルは
...
- OPENMC_PYPI_SOEXT: 共有ライブラリ拡張子 (dll / so / dylib)。dll なら実行ファイルは .exe。
- OPENMC_PYPI_NJOY:  njoy 実行ファイルのパス。未設定なら njoy 無し wheel (darwin の
  flang クロスが通らない場合の逃げ道。makefile の WITH_NJOY=0)。
...
class CustomBuildHook(BuildHookInterface):
    def initialize(self, version, build_data):
        ...
        out = os.environ["OPENMC_PYPI_OUT"]
        suffix = os.environ["OPENMC_PYPI_SOEXT"]
        exe_ext = ".exe" if suffix == "dll" else ""
        lib = f"{out}/libopenmc.{suffix}"
        exe = f"{out}/openmc{exe_ext}"
        njoy = os.environ.get("OPENMC_PYPI_NJOY")
